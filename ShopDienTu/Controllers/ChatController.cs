    ...
    public class ChatController : Controller
    {
        ...
        public async Task<JsonResult> Chat([FromBody] ChatRequest request)
        {
            try
            {
                ...
                var systemPrompt = "Bạn là trợ lý của trang web ShopDienTu, chuyên bán các sản phẩm điện tử như laptop, điện thoại, phụ kiện, và linh kiện máy tính. " +
                    "Trả lời câu hỏi của người dùng về sản phẩm, giá cả, khuyến mãi, danh mục, và cách sử dụng sản phẩm. " +
                    "Định dạng câu trả lời bằng Markdown với danh sách sản phẩm dùng dấu đầu dòng (*), tiêu đề sản phẩm in đậm (**), giá in nghiêng (*), và khuyến mãi nổi bật. " +
                    "Thêm đường dẫn ảnh chính của sản phẩm trong định dạng ![Tên sản phẩm](Đường dẫn ảnh) nếu có. " +
                    "Trả lời thân thiện, chuyên nghiệp, và sử dụng tiếng Việt. Nếu không tìm thấy thông tin, hãy trả lời rằng thông tin không khả dụng.";
                ...
                var productInfo = string.Join("\n", products.Select(p =>
                    ...
                    (string.IsNullOrEmpty(p.MainImage) ? "" : $" ![{p.ProductName}](/images/{p.MainImage})")));
                ...
                string markdownResponse = result.candidates[0].content.parts[0].text;
